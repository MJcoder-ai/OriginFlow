...
- **Chat Sidebar**: Dedicated panel for collaborating with the AI assistant, now a mission-control centre. It displays a high-level Plan Timeline at the top of the chat history, presents AI suggestions as rich Design Cards, and adds a Quick-Action Bar and Mode Selector above the input. Conversation history persists across sessions, and the AI proactively notifies you when required components are missing or guides you to upload datasheets or choose alternatives. Quick-action links in the chat can automatically apply filters or search terms in the Component Library.
  - **Plan Timeline**: A dynamic timeline summarises the AI’s tasks (e.g. selecting panels, sizing wiring) with status icons that indicate whether each task is pending, in progress, complete or blocked. These tasks are generated by a new planning endpoint (`POST /api/v1/ai/plan`) that decomposes your command into high‑level steps. The timeline collapses away when there are no active tasks.
