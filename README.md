...
- **Variant Splitting & Memory Logging**: Multi-option datasheets are split into individual variant records, and each AI command is logged with auto-approved and pending action counts linked to its trace ID.
...
- **Chat Sidebar**: Dedicated panel for collaborating with the AI assistant. Includes voice mode input and persists conversation history.  The AI now proactively uses this chat to notify you when required components are missing and to guide you to upload datasheets or select from available alternatives.  Quick‑action links in the chat can automatically apply filters or search terms in the Component Library.
