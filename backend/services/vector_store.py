...
class NoOpVectorStore:
    """Fallback vector store that performs no operations."""
...
def get_vector_store() -> VectorStore:
    ...
    if backend == "chroma":
        ...
        except Exception as exc:  # pragma: no cover - depends on chroma installation
            logger.warning("Chroma vector store unavailable: %s", exc)
            return NoOpVectorStore()
    try:
        return QdrantVectorStore()
    except Exception as exc:  # pragma: no cover - depends on qdrant installation
...
class QdrantVectorStore:
    ...
    def __init__(
        ...
        self.collection_name = collection_name or os.getenv(
            "QDRANT_COLLECTION", "ai_action_vectors"
        )
    ...
    async def search(
        ...
        if filters:
            conds = [
                rest.FieldCondition(key=key, match=rest.MatchValue(value=val))
    ...
    async def delete(self, id: str) -> None:
        self.client.delete(
            collection_name=self.collection_name,
            points_selector=rest.PointIdsList(points=[id]),
        )
...
class ChromaVectorStore:
    ...
    def __init__(
        ...
        self.collection_name = collection_name or os.getenv(
            "CHROMA_COLLECTION", "ai_action_vectors"
        )
